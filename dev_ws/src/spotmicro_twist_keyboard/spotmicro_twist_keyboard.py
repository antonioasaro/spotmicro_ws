...
class SpotMicroKeyboardControl():
    def __init__(self):
        ...
        global node
        node = rclpy.create_node('spotmicro_twist_keyboard')
        node.get_logger().info("Setting Up the Spot Micro Keyboard Control Node...")
        ...
        node.get_logger().info("Keyboard control node initialization complete")
    ...
    def run(self):
        ...
        node.get_logger().info('Main keyboard control loop started.')
        ...
        while (1):
            ...
            if userInput not in valid_cmds:
                node.get_logger().warn('Invalid keyboard command entered: ' + userInput)
            else:
                if userInput == 'quit':
                    node.get_logger().info("Exiting keyboard control node...")
                    break
                    ...
                    node.get_logger().info('Entering keyboard angle command mode.')
                    ...
                    while (1):
                        ...
                        if userInput == 'u':
                            ...
                            node.get_logger().warn('Invalid keyboard command issued in angle command mode: %s' + userInput)
                        else:
                            if userInput == 'w':
                    ...
                    node.get_logger().info('Walk command issued from keyboard.')
                    ...
                    while (1):
                        ...
                        if userInput == 'u':
                            ...
                            if userInput == 'w':
                                ...
                                self._ros_pub_vel_cmd.publish(self._vel_cmd_msg)
                                node.get_logger().info('Command issued to zero all rate commands.')
